...
export const DitherShader = ({
    ...
    const applyDithering = useCallback(
        (ctx, displayWidth, displayHeight, time = 0) => {
            ...
            const effectivePixelSize = Math.max(
                1,
                Math.floor(gridSize * pixelRatio),
            );
            const matrixSize = gridSize <= 4 ? 4 : 8;
            const bayerMatrix =
                gridSize <= 4 ? BAYER_MATRIX_4x4 : BAYER_MATRIX_8x8;
            ...
            for (let y = 0; y < displayHeight; y += effectivePixelSize) {
                for (let x = 0; x < displayWidth; x += effectivePixelSize) {
                    ...
                    r = clamp(
                        (r - 128) * contrast + 128 + brightness * 255,
                        0,
                        255,
                    );
                    g = clamp(
                        (g - 128) * contrast + 128 + brightness * 255,
                        0,
                        255,
                    );
                    b = clamp(
                        (b - 128) * contrast + 128 + brightness * 255,
                        0,
                        255,
                    );
                    ...
                    switch (ditherMode) {
                        case "bayer":
                            ditherThreshold =
                                bayerMatrix[matrixY][matrixX] / matrixScale;
                            break;
                        case "halftone": {
                        ...
                        case "crosshatch": {
                            const line1 =
                                (x + y) % (gridSize * 2) < gridSize ? 1 : 0;
                            const line2 =
                                (x - y + gridSize * 4) % (gridSize * 2) <
                                gridSize
                                    ? 1
                                    : 0;
                    ...
                    switch (colorMode) {
                        ...
                        default: {
                            ...
                            const adjustedR = clamp(
                                r + ditherAmount * 64,
                                0,
                                255,
                            );
                            const adjustedG = clamp(
                                g + ditherAmount * 64,
                                0,
                                255,
                            );
                            const adjustedB = clamp(
                                b + ditherAmount * 64,
                                0,
                                255,
                            );
                            ...
                            const levels = 4;
                            outputColor = [
                                Math.round(adjustedR / (255 / levels)) *
                                    (255 / levels),
                                Math.round(adjustedG / (255 / levels)) *
                                    (255 / levels),
                                Math.round(adjustedB / (255 / levels)) *
                                    (255 / levels),
                            ];
                            break;
                        }
